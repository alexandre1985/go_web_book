...
// reading key files before starting http handlers
func init() {
	var err error
...
func loginHandler(w http.ResponseWriter, r *http.Request) {
	...
	if user.UserName != "daniel" && user.Password != "pass" {
...
func authHandler(w http.ResponseWriter, r *http.Request) {
	...
	token, err := request.ParseFromRequest(r, request.OAuth2Extractor, func(token *jwt.Token) (interface{}, error) {
		// since we only use one private key to sign the tokens, we also only use its public counterpart to verify
	...
	if err != nil {
		...
		case *jwt.ValidationError: // something was wrong during the validation
			...
			case jwtValidationErrorExpired:
